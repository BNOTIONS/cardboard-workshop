...
public class MainActivity extends CardboardActivity implements CardboardView.StereoRenderer {
    ...
    private List<float[]> mModelCubeList = new LinkedList<float[]>();
    ...
    private void makeCube(float xPadding, float yPadding, float zPadding) {
        float[] cube = new float[16];
        // New cubes first appears directly in front of user
        Matrix.setIdentityM(cube, 0);
        Matrix.translateM(cube, 0, xPadding, -(mFloorDepth / 2) + yPadding, -mObjectDistance + zPadding);

        mModelCubeList.add(cube);
    }
    ...
    public void onNewFrame(HeadTransform headTransform) {
        ...
        for (float[] cube : mModelCubeList) {
            Matrix.rotateM(cube, 0, TIME_DELTA, 0.5f, 0.5f, 1.0f);
        }
    ...
    public void onDrawEye(EyeTransform transform) {
        ...
        for (float[] cube : mModelCubeList) {
            Matrix.multiplyMM(mModelView, 0, mView, 0, cube, 0);
     ...
     * the shader.
     */
    public void drawCube(float[] cube) {
        // This is not the floor!
        GLES20.glUniform1f(mIsFloorParam, 0f);

        // Set the Model in the shader, used to calculate lighting
        GLES20.glUniformMatrix4fv(mModelParam, 1, false, cube, 0);
     ...
     */
    private void respawnOutOfSight(float[] cube) {
        ...
        Matrix.multiplyMV(posVec, 0, rotationMatrix, 0, cube, 12);
    ...
    private boolean isLookingAtObject(float[] cube) {
        ...
        Matrix.multiplyMM(mModelView, 0, mHeadView, 0, cube, 0);
